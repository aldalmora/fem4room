...
from fem4room import SaveLoad,Sources,Visualization,Other,Boundary,Solver,TimeEngine
...
c=340.
Z = 3 + 5j #TODO: For low impedance the reference is not right, in the plot
...
h=np.min(wavelength)/20 #At least a certain amount of elements per wavelength
impedance_pg=1
source_pg=2
...
M_2d,nodeTags_imp = Boundary.Impedance_Damping_Matrix(m,impedance_pg)
...
plt.axhline(np.max(s[:,idx_xR])/(1+R),c='b')
plt.xlabel('t(s)')
plt.ylabel('Pressure')
